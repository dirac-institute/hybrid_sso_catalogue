...
Near-Earth Objects (NEOs) are objects that may come close enough to the Earth that small perturbations in their orbit lead to a collision course. A simple definition for NEOs is that there are small solar system bodies that have a perihelion less than $1.3 \unit{AU}$ \needcite{}. Given the threat posed by these objects, it is therefore critical that we catalogue and determine the orbit of NEOs.
...
The Legacy Survey of Space and Time (LSST) will rapidly increase the rate at which NEOs are discovered and sent to the NEOCP. \todo{need more details}
...
This paper quanitifies the impact of LSST on the NEOCP and the fraction of objects that require community follow-up.

Our paper is structured as follows. In Section~\ref{sec:method}, we describe the methods for creating a hybrid catalogue of objects, simulating LSST observations and calculating \dig{} scores for the NEOCP. \todo{Outline rest of paper}
...
To complete the merging process, we compile the matched object IDs and delete them from \sss{}. We then add the entirety of \mpco{} to the remaining catalogue, resulting in a hybrid catalogue. We assess the quality of the hybrid catalogue in Appendix~\ref{app:hybrid_quality}.
...
\begin{figure*}[htb]
    \centering
    \includegraphics[width=\textwidth]{prediction_algorithm.png}
    \caption{A demonstration of the prediction algorithm described in Section~\ref{sec:pred_alg}. \textbf{Left:} A confusion matrix representing the algorithm's ability to predict the detectability of an object. \textbf{Right:} A distribution of assigned probabilities, split into a population of objects that are actually detected and a population that are not.}
\end{figure*}
...
\section{Assessing quality of hybrid catalogue}\label{app:hybrid_quality}
