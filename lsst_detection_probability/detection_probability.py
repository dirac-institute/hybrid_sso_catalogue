...
def get_detection_probabilities(night_start, path="../neocp/neo/", detection_window=15, min_nights=3):
    ...
    shifted = shifted.rename(columns={"fieldRA": "previousFieldRA", "fieldDec": "previousFieldDec", "night": "previousNight"})
    ...
    for i, night in enumerate(night_list):
        ...
        if not full_schedule[mask].empty:
            night_lengths[i] = full_schedule[mask].iloc[-1]["observationStartMJD"] - full_schedule[mask].iloc[0]["observationStartMJD"]

    # get the first visit from each night
    first_visit_each_night = full_schedule[full_schedule["night"] != full_schedule["previousNight"]]
    ...
    # get the truth
    unique_findable_neo_hex_ids = np.load("../neocp/unique_findable_neo_hex_ids_linked.npy", allow_pickle=True)
    ...
    for i, hex_id in enumerate(unique_objs):
        probs[i] = probability_from_id(hex_id, sorted_obs, distances=np.logspace(-1, 1.1, 50) * u.AU,
                                       ...
                                       first_visit_each_night=first_visit_each_night,
                                       full_schedule=full_schedule, night_lengths=night_lengths,
                                       night_list=night_list, detection_window=detection_window,
...
def probability_from_id(hex_id, sorted_obs, distances, radial_velocities, first_visit_each_night,
    ...
    start_orbits = variant_orbit_ephemerides(ra=rows.iloc[0]["AstRA(deg)"] * u.deg,
                                             ...
                                             eph_times=Time(first_visit_each_night["observationStartMJD"].values, format="mjd"))
    ...
    for j in range(len(start_orbits)):
        delta_ra = start_orbits.loc[j]["vRAcosDec"] / np.cos(start_orbits.loc[j]["Dec_deg"] * u.deg) * night_lengths[j]
        ...
        mask = full_schedule["night"] == night
        masked_schedules[night_list.index(night)] = full_schedule[mask][np.logical_and.reduce((full_schedule[mask]["fieldRA"] > ra_lims[0],
